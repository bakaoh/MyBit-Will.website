...
export default class RedeemPage extends React.Component {
  ...
  async removeTransactionFromProcess(contractAddress){
  ...
  async handleReceive(contractAddress){
    ...
    try{
      ...
      this.removeTransactionFromProcess(contractAddress);
    }
    setTimeout( async () => {
      this.removeTransactionFromProcess(contractAddress);
    }, 4000);
  ...
  render() {
    ...
    return (
      <div>
        <Helmet>
          <title>Redeem - MyBit Trust</title>
          <meta
            name="Redeem"
            content="Redeem your funds on the MyBit Trust dApp"
          />
        </Helmet>
        <ConnectionStatus
